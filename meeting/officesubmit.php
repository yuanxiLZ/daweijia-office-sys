<?php
	header("Content-type: text/html;charset=utf-8");
 include("../sqlconnect.php"); 
 include("../wechat/wechatfunction.php");

  session_start();
	    $id=$_SESSION['userID'];
    	$name=$_SESSION['name'];
   		$department=$_SESSION['department'];
    	$position=$_SESSION['position'];
    	$LV=$_SESSION['LV'];
      $accesstoken=$_SESSION['access_token'];
      $openid=$_SESSION['openid'];
      $meetingroom=$_SESSION['meetingroom']
      // if ($LV==2) {
      //   header('Location:checkmeeting2.php');
      //   //$_SESSION['sealid']=$_GET['sealid'];
      // }


?>
<!DOCTYPE html>
<html>
<head>
	 
   <meta charset="UTF-8">
  
   <title>智慧大魏家</title>
   <link rel="icon" type="image/png" href="../img/logo.png">
  <script src="../assets/js/jquery-1.10.2.js" type="text/javascript"></script>
  <script src="../assets/js/jquery-ui-1.10.4.custom.min.js" type="text/javascript"></script>
<!--   <script src="../js/mobiscroll.custom.min.js" type="text/javascript"></script> -->
  <script src="../bootstrap3/js/bootstrap.js" type="text/javascript"></script>
   <meta content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0' name='viewport' />
  <!--  Plugins -->
  <!-- <script src="../assets/js/ct-paper-checkbox.js"></script>
  <script src="../assets/js/ct-paper-radio.js"></script> -->
  <script src="../assets/js/bootstrap-select.js"></script>
  <script src="../assets/js/bootstrap-datepicker.js"></script>
  <script type="text/javascript" src='../js/selfdefin_function.js'></script>
  <script src="../assets/js/ct-paper.js"></script>    
  <script type="text/javascript" src='../js/test.js'></script>

  <script type="text/javascript" src='../js/mobiscroll.custom.min.js'></script>
  <!-- css-->

    <link href="../bootstrap3/css/bootstrap.css" rel="stylesheet" />
    
    <link href="../assets/css/demo.css" rel="stylesheet" /> 
   <!-- · -->

</head>

<body>
<nav class="navbar navbar-ct-transparent" role="navigation-demo" id="demo-navbar">
  <div class="container">
    <!-- Brand and toggle get grouped for better mobile display -->
    <div class="navbar-header">
  
      
           <div class="logo-container">
                
               
                   <?php
           echo "<h4>";
            echo $name,"-";
            echo $department,"-";
            echo $position,"-";
           echo "</h4>"
          ?>
            
            
             <a href="../index.php"><button  class="btn btn-default btn-lg">主页</button></a>
            
            </div>
      
    </div>

<!-- Collect the nav links, forms, and other content for toggling -->
    <div class="collapse navbar-collapse" id="navigation-example-2">
      <ul class="nav navbar-nav navbar-right">
         
          
       </ul>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-->
</nav>         
<a href="workerpage.php">工作人员入口</a>
<?php  
if (!empty($_GET['meetingroom'])) {
  # code...
  $meetingroom=$_GET['meetingroom'];
}
if (!$link){
    echo"alert('数据库连接失败！')";
}else {
      
        $query = "UPDATE meetingroom SET meetingroom = '{$meetingroom}', meetingoffice = '{$_GET['decision']}' WHERE meetingID = '{$_GET['meetingID']}'";
    if (mysqli_query($link, $query)){
        echo "success";
        
        if ($_GET['decision']==1) {
          # code...
          $query="select * from meetingroom where meetingID = '{$_GET['meetingID']}'";
        $result = mysqli_query($link, $query);
        $row = mysqli_fetch_array($result);
        $meetingmember=$row['meetingmemberID']."00009_";
        $meetinglist = explode('_',$meetingmember);
        var_dump($meetinglist);
      
        for($index=0;$index<(count($meetinglist)-1);$index++) 
            { 
              echo "array";
              echo $index;
              echo $meetinglist[$index];
              $meetingtime=$row['meetingdate']." ".$row['meetingstart'];
              sendmission($link,$accesstoken,$meetinglist[$index],$meetingtime,$_GET['meetingID'],"会议",$row['meetingroom'],$row['meetingcontent'],"www.daweijia.cn/daweijia/demo/meeting/meetingnote.php?meetingid={$row['meetingID']}");
            } 
        }
         sendstarter($link,$accesstoken,$row['meetingstarter'],"会议",$name,$_GET['decision'],"","");
     }else{
         echo("Errorcode: " . mysqli_errno($link));
     }
     
        
//         if (!mysqli_query($link, $query))
// {
// echo("Errorcode: " . mysqli_errno($con));
// }
//     }
    
}


?>
<!-- <form action="sealcheck.php" method="get">
  
   <?php
   //echo " <input type='text' name='sealid' style='display:none' value='{$_GET['sealid']}'> ";

   ?>
   建议：
    <textarea rows="6" cols="50" name="suggestion">`

    </textarea>
   <select name="decision" >
            <option value="1">同意</option>
            <option value="-1">不同意</option>
            <option value="" selected="selected">是否同意</option>
    </select>
<input type="submit" name="submit" value="Submit" />
</form> -->

<footer>
	版权所有 2017-<?php echo date("Y")?> all right resverved

</footer>
</body>
</html>