    ...
    echo"alert('数据库连接失败！')";
        ...
        $row = mysqli_fetch_array($result);
          ...
          for ($x=0; $x<(sizeof($spersonid)-1); $x++){
                  ...
                  $answer=$answer."0_";
                  $checkid=$row['sperson']."00005_";
               
          
          $query= "UPDATE seal SET ifs = '{$answer}',suggestion ='{$_GET['suggestion']}',sperson= '$checkid' WHERE sealid = '{$_GET['sealid']}' "; 
           if (mysqli_query($link, $query)){
        echo "success";
         if (strlen($nextid)>1&&$_POST["decision"]=='1') {//wechat
          # code...
         
          sendcheck($link,$accesstoken,$nextid,"印章",$row['type'],$row['title'],$row['sdate'],$row['starter'],"123.207.96.71/daweijia/demo/checkseal.php?sealid={$_GET['sealid']}");


        }
       sendstarter($link,$accesstoken,$row['starter'],"印章",$name,$row['type'],$_GET['decision'],$_GET['suggestion'],"");
     }else{
