...
&& in_array($extension, $allowedExts))
{
    ...
    else
    {
        ...
        if (file_exists("../receive_file/" . $_FILES["file"]["name"]))
        {
        ...
        else
        {
            ...
            move_uploaded_file($_FILES["file"]["tmp_name"], "../receive_file/" . $file_name);
...
include("../wechat/wechatfunction.php");//wechat

if (!$link){
    echo"alert('数据库连接失败！')";
}else {
      
        $query = "INSERT INTO acceptfile (title,uploaddate,address,maincontent,checkname,checkid,fileroad,checkstatus,starter,starterid,fileid,safelv,acceptdate,urgencylv)VALUES('{$_POST['title']}','$today','{$_POST['address']}','{$_POST['content']}','{$_POST['checkname']}','$checkidtxt','../receive_file/$file_name','$checkstatus','$name','$id','{$_POST['fileid']}','{$_POST['safelv']}','{$_POST['date']}','{$_POST['urgencylv']}')";
        ...
        echo $nextid[0];
        sendcheck($link,$accesstoken,$nextid[0],"收文",$_POST['title'],$_POST['content'],$_POST['safelv'],$name,"123.207.96.71/daweijia/demo/receive/check.php?afid={$oid}");
     }else{
