    ...
    echo"alert('数据库连接失败！')";
       ...
       while($row = mysqli_fetch_array($result))
      {
          ...
          for ($x=0; $x<(sizeof($spersonid)-1); $x++){
                  ...
                  $suggestion="$name".":".$row['suggest'].$_GET['suggestion'];
                  echo $answer;
               
          }
          $query= "UPDATE holiday SET checkstatus = '{$answer}',suggest ='$suggestion' WHERE id = '{$_GET['leaveid']}' "; 
        ...
        if (strlen($nextid)>1&&$_POST["decision"]=='1') {//wechat
          ...
          sendcheck($link,$accesstoken,$nextid,"请假",$row['title'],$row['maincontent'],$row['safelv'],$name,"123.207.96.71/daweijia/demo/leave/leave_check.php?leaveid={$_GET['leaveid']}");


        }
      
        sendstarter($link,$accesstoken,$row['starter'],"请假",$name,$_GET['decision'],$_GET['suggestion'],"");
