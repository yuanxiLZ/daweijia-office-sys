...
if (!$link){
    echo"alert('数据库连接失败！')";
        ...
        $query_meeting = "select * from meetingroom where meetingoffice = '1' and meetingdate >= '$today'  order by meetingdate desc";
        $result_meeting = mysqli_query($link, $query_meeting);
          while($row = mysqli_fetch_array($result_meeting))
      {
          echo  "会议开始时间".$row['meetingdate']."_".$row['meetingstart'];
          echo  "会议时长".$row['meetingtime']."_";
          echo  "会议室：".$row['meetingroom'];
